...
class SDPolicy(TorchStochasticSequencePolicy):
    ...
    def __init__(
        ...
        self.std = std
        in_dim = self.total_encode_dim * 2
        if self.attach_obs:
            in_dim += obs_dim
        ...
        if layer_norm:
            self.layer_norm = torch.nn.LayerNorm(2 * self.total_encode_dim)
        else:
...
class SDPolicyAdapter(TorchStochasticPolicy):
    ...
    def forward(self, obs):
        ...
        if self.policy.layer_norm is not None:
            curr_sd = self.policy.layer_norm(curr_sd)
