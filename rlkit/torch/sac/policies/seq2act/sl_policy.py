"""
Sequential policy that does linear transformation over statistics.
...
class SLPolicy(TorchStochasticSequencePolicy):
...
class SLPolicyAdapter(TorchStochasticPolicy):
